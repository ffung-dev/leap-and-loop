...
export function ExploreCta() {
  return (
    <section className="gradient-card border-t border-tan-200">
      <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <h2 className="font-display text-3xl text-brown-900 sm:text-4xl">Keep exploring</h2>
        <div className="mt-8 grid gap-5 sm:grid-cols-3">
          {CARDS.map(({ href, icon: Icon, title, description }) => (
            <Link
              key={href}
              href={href}
              className="group flex flex-col gap-3 rounded-3xl border border-tan-200 bg-white p-6 shadow-sm transition-shadow hover:shadow-md"
            >
